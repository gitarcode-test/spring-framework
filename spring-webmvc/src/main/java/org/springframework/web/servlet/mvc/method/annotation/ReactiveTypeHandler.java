...
class ReactiveTypeHandler {
  ...
  private static final boolean isContextPropagationPresent =
  ...
  @Nullable private final Object contextSnapshotHelper;
  ...
  public ResponseBodyEmitter handleValue(
    ...
    if (adapter.isMultiValue()) {
      if (mediaTypes.stream().anyMatch(MediaType.TEXT_EVENT_STREAM::includes)
        ...
        new SseEmitterSubscriber(emitter, this.taskExecutor, taskDecorator)
  ...
  /**
   * Attempts to find a concrete {@code MediaType} that can be streamed (as json separated by
   * newlines in the response body). This method considers two concrete types {@code
   * APPLICATION_NDJSON} and {@code APPLICATION_STREAM_JSON}) as well as any subtype of application
   * that has the {@code +x-ndjson} suffix. In the later case, the media type MUST be concrete for
   * it to be considered.
   *
   * <p>For example {@code application/vnd.myapp+x-ndjson} is considered a streaming type while
   * {@code application/*+x-ndjson} isn't.
   *
   * @param acceptedMediaTypes the collection of acceptable media types in the request
   * @return the concrete streaming {@code MediaType} if one could be found or {@code null} if none
   *     could be found
   */
  ...
  private static class SseEmitterSubscriber extends AbstractEmitterSubscriber {
    ...
    SseEmitterSubscriber(
        SseEmitter sseEmitter, TaskExecutor executor, @Nullable TaskDecorator taskDecorator) {
