...
public abstract class AbstractCachingViewResolver extends WebApplicationObjectSupport
   ...
   *
   * <p>Default is "true": unresolved view names are cached. Note that this flag only applies if the
   * general {@link #setCache "cache"} flag is kept at its default of "true" as well.
   *
   * <p>Of specific interest is the ability for some {@link AbstractUrlBasedView} implementations
   * (e.g., FreeMarker) to check if an underlying resource exists via {@link
   * AbstractUrlBasedView#checkResource(Locale)}. With this flag set to "false", an underlying
   * resource that re-appears is noticed and used. With the flag set to "true", only one check is
   * made.
   */
   ...
   *
   * <p>This can be handy in case developers are able to modify views (e.g., FreeMarker templates)
