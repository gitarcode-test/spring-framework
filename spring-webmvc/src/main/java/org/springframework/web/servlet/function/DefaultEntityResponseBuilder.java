...
final class DefaultEntityResponseBuilder<T> implements EntityResponse.Builder<T> {
  ...
  public EntityResponse<T> build() {
    if (this.entity instanceof CompletionStage completionStage) {
      ...
      ReactiveAdapter adapter =
          ReactiveAdapterRegistry.getSharedInstance().getAdapter(this.entity.getClass());
      if (adapter != null) {
  ...
  private static class DefaultEntityResponse<T> extends AbstractServerResponse
  ...
  private static class CompletionStageEntityResponse<T>
    ...
    protected ModelAndView writeToInternal(
      ...
      DeferredResult<ServerResponse> deferredResult =
          createDeferredResult(servletRequest, servletResponse, context);
  ...
  private static class PublisherEntityResponse<T> extends DefaultEntityResponse<Publisher<T>> {
      ...
      entity()
          .subscribe(
              new DeferredResultSubscriber(
                  servletRequest, servletResponse, context, deferredResult));
      return null;
    }
    ...
    private class DeferredResultSubscriber implements Subscriber<T> {
      ...
      public DeferredResultSubscriber(
      ...
      public void onNext(T t) {
        ...
        try {
          tryWriteEntityWithMessageConverters(
              t, this.servletRequest, this.servletResponse, this.context);
