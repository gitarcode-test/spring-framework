...
class BindingContextTests {
  ...
  void bindUriVariablesAndHeaders() {
    ...
    exchange
        .getAttributes()
        .put(HandlerMapping.URI_TEMPLATE_VARIABLES_ATTRIBUTE, Map.of("name", "John", "age", "25"));

    TestBean target = new TestBean();

    BindingContext bindingContext = new BindingContext(null);
    WebExchangeDataBinder binder =
        bindingContext.createDataBinder(exchange, target, "testBean", null);
  ...
  void bindUriVarsAndHeadersAddedConditionally() {
    ...
    exchange
        .getAttributes()
        .put(HandlerMapping.URI_TEMPLATE_VARIABLES_ATTRIBUTE, Map.of("age", "26"));
  ...
  private record WrappedBeanValidator(jakarta.validation.Validator validator)
