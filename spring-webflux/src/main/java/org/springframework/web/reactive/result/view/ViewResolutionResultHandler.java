 ...
 *
 * <ul>
 *   <li>{@link Void}, {@code void}, or no value -- default view name
...
public class ViewResolutionResultHandler extends HandlerResultHandlerSupport
      ...
      if (adapter.isMultiValue()) {
        valueMono =
            (result.getReturnValue() != null
                ? Mono.just(
                    FragmentsRendering.withPublisher(adapter.toPublisher(result.getReturnValue()))
                        .build())
                : Mono.empty());
        ...
        .flatMap(
            returnValue -> {
              ...
              if (Collection.class.isAssignableFrom(clazz)) {
                returnValue =
                    FragmentsRendering.withCollection((Collection<Fragment>) returnValue).build();
              ...
              if (returnValue == NO_VALUE || ClassUtils.isVoidType(clazz)) {
                ...
                viewsMono =
                    (view instanceof String viewName
                        ? resolveViews(viewName, locale)
                ...
                Flux<Flux<DataBuffer>> renderFlux =
                    render
                        .fragments()
                        .concatMap(
                            fragment -> renderFragment(fragment, locale, bindingContext, exchange));
  ...
  private Mono<Flux<DataBuffer>> renderFragment(
    ...
    Mono<List<View>> selectedViews =
        (fragment.isResolved()
            ? Mono.just(List.of(fragment.view()))
            : resolveViews(
                fragment.viewName() != null ? fragment.viewName() : getDefaultViewName(exchange),
                locale));

    FragmentFormatter fragmentFormatter = getFragmentFormatter(exchange);

    return selectedViews
        .flatMap(views -> render(views, fragment.model(), bindingContext, mutatedExchange))
        .then(
            Mono.fromSupplier(
                () ->
                    (fragmentFormatter != null
  ...
  private static class BodySavingResponse extends ServerHttpResponseDecorator {
    ...
    @Nullable private Flux<DataBuffer> bodyFlux;
