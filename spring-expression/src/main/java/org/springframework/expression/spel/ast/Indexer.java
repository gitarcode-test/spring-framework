...
/**
 * An {@code Indexer} can index into some proceeding structure to access a particular element of the
 * structure.
 *
 * <p>Numerical index values are zero-based, such as when accessing the n<sup>th</sup> element of an
 * array in Java.
 *
 * <h3>Supported Structures</h3>
 *
 * <ul>
 *   <li>Arrays: the n<sup>th</sup> element
 *   <li>Collections (lists, sets, etc.): the n<sup>th</sup> element
 *   <li>Strings: the n<sup>th</sup> character as a {@link String}
 *   <li>Maps: the value for the specified key
 *   <li>Objects: the property with the specified name
 *   <li>Custom Structures: via registered {@link IndexAccessor} implementations
 * </ul>
 *
 * <h3>Null-safe Indexing</h3>
 *
 * <p>As of Spring Framework 6.2, null-safe indexing is supported via the {@code '?.'} operator. For
 * example, {@code 'colors?.[0]'} will evaluate to {@code null} if {@code colors} is {@code null}
 * and will otherwise evaluate to the 0<sup>th</sup> color.
 *
 * @author Andy Clement
 * @author Phillip Webb
...
public class Indexer extends SpelNodeImpl {
  ...
  @Nullable private volatile String originalPrimitiveExitTypeDescriptor;

  @Nullable private volatile String arrayTypeDescriptor;

  @Nullable private volatile CachedPropertyState cachedPropertyReadState;

  @Nullable private volatile CachedPropertyState cachedPropertyWriteState;

  @Nullable private volatile CachedIndexState cachedIndexReadState;

  @Nullable private volatile CachedIndexState cachedIndexWriteState;

  /**
   * Create an {@code Indexer} with the given start position, end position, and index expression.
   *
   * @see #Indexer(boolean, int, int, SpelNodeImpl)
   * @deprecated as of Spring Framework 6.2, in favor of {@link #Indexer(boolean, int, int,
   *     SpelNodeImpl)}
   */
  ...
  /**
   * Create an {@code Indexer} with the given null-safe flag, start position, end position, and
   * index expression.
   *
   * @since 6.2
   */
  ...
  private ValueRef getValueRef(ExpressionState state, AccessMode accessMode)
      throws EvaluationException {
    ...
    if (target.getClass().isArray()) {
      ...
      return new ArrayIndexingValueRef(
          state.getTypeConverter(), target, intIndex, targetDescriptor);
    }
    ...
    if (accessMode.supportsReads) {
      try {
        ...
        throw new SpelEvaluationException(
            getStartPosition(),
            ex,
            SpelMessage.EXCEPTION_DURING_INDEX_READ,
            index,
    ...
    if (accessMode.supportsWrites) {
      try {
        ...
        throw new SpelEvaluationException(
            getStartPosition(),
            ex,
            SpelMessage.EXCEPTION_DURING_INDEX_WRITE,
            index,
            target.getClass().getTypeName());
      }
    }
  ...
  public void generateCode(MethodVisitor mv, CodeFlow cf) {
    ...
    if (this.indexedType == IndexedType.ARRAY) {
      ...
      if (!(cachedPropertyReadState.accessor
          instanceof CompilablePropertyAccessor compilablePropertyAccessor)) {
        throw new IllegalStateException(
      ...
      if (!(cachedIndexReadState.accessor
          instanceof CompilableIndexAccessor compilableIndexAccessor)) {
        throw new IllegalStateException(
  ...
  private void generateIndexCode(
      MethodVisitor mv, CodeFlow cf, SpelNodeImpl indexNode, Class<?> indexType) {
   ...
   *
   * <p>If the current target type and property name match these values, the cached {@code
   * PropertyAccessor} is used to access the property.
   *
   * <p>If they do not match, a suitable {@code PropertyAccessor} is discovered and cached for later
   * use.
   */
   ...
   *
   * <p>If the current target type and index match these values, the cached {@code IndexAccessor} is
   ...
   * @param index the index value: the value inside the square brackets, such as the Integer 0 in
   *     [0], the String "name" in ['name'], etc.
   */
  ...
  private class PropertyAccessorValueRef implements ValueRef {
    ...
    public PropertyAccessorValueRef(
        Object targetObject,
        String name,
        ...
        for (PropertyAccessor accessor : accessorsToTry) {
          if (accessor.canRead(this.evaluationContext, this.targetObject, this.name)) {
            if (accessor instanceof ReflectivePropertyAccessor reflectivePropertyAccessor) {
              accessor =
                  reflectivePropertyAccessor.createOptimalAccessor(
            ...
            Indexer.this.cachedPropertyReadState =
                new CachedPropertyState(accessor, targetType, this.name);
            if (accessor instanceof CompilablePropertyAccessor compilablePropertyAccessor) {
              setExitTypeDescriptor(
                  CodeFlow.toDescriptor(compilablePropertyAccessor.getPropertyType()));
            }
            return result;
          }
        }
      ...
      try {
        ...
        for (PropertyAccessor accessor : accessorsToTry) {
          if (accessor.canWrite(this.evaluationContext, this.targetObject, this.name)) {
            accessor.write(this.evaluationContext, this.targetObject, this.name, newValue);
            Indexer.this.cachedPropertyWriteState =
                new CachedPropertyState(accessor, targetType, this.name);
            return;
          }
        }
      ...
      try {
        ...
        for (IndexAccessor indexAccessor : accessorsToTry) {
          if (indexAccessor.canRead(this.evaluationContext, this.target, this.index)) {
            ...
            Indexer.this.cachedIndexReadState =
                new CachedIndexState(indexAccessor, targetType, this.index);
            if (indexAccessor instanceof CompilableIndexAccessor compilableIndexAccessor) {
              setExitTypeDescriptor(
                  CodeFlow.toDescriptor(compilableIndexAccessor.getIndexedValueType()));
            }
            return result;
          }
        }
      ...
      try {
        ...
        for (IndexAccessor indexAccessor : accessorsToTry) {
          if (indexAccessor.canWrite(this.evaluationContext, this.target, this.index)) {
            ...
            Indexer.this.cachedIndexWriteState =
                new CachedIndexState(indexAccessor, targetType, this.index);
            return;
          }
        }
