 ...
 *
 * <p>Functions can be either a {@link Method} (for static Java methods) or a {@link MethodHandle}
 * and must be registered in the context prior to evaluation of the expression. See the {@code
 * registerFunction()} methods in {@link
 * org.springframework.expression.spel.support.StandardEvaluationContext} for details.
 *
 * @author Andy Clement
 ...
 * @since 3.0
 * @see
 *     org.springframework.expression.spel.support.StandardEvaluationContext#registerFunction(String,
 *     Method)
 * @see
 *     org.springframework.expression.spel.support.StandardEvaluationContext#registerFunction(String,
 *     MethodHandle)
...
public class FunctionReference extends SpelNodeImpl {
   ...
   */
  private TypedValue executeFunctionViaMethod(ExpressionState state, Method method)
    ...
    try {
      ...
      Throwable cause =
          ((ex instanceof InvocationTargetException ite && ite.getCause() != null)
              ? ite.getCause()
              : ex);
      throw new SpelEvaluationException(
          getStartPosition(),
   ...
   *
   * <p>Method types that take no arguments (fully bound handles or static methods with no
   * parameters) can use {@link MethodHandle#invoke(Object...)} which is the most efficient.
   * Otherwise, {@link MethodHandle#invokeWithArguments(Object...)} is used.
   *
   * @param state the expression evaluation state
   ...
   * @since 6.1
   */
  private TypedValue executeFunctionViaMethodHandle(
      ExpressionState state, MethodHandle methodHandle) throws EvaluationException {
    Object[] functionArgs = getArguments(state);
    ...
    ReflectionHelper.convertAllMethodHandleArguments(
        converter, functionArgs, methodHandle, varArgPosition);
    ...
    if (isSuspectedVarargs) {
      if (declaredParamCount == 1) {
        ...
        if (actualVarargsIndex >= 0
            && functionArgs[actualVarargsIndex] instanceof Object[] argsToUnpack) {
