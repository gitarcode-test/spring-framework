 ...
 *
 * <ul>
 *   <li><code>new example.Foo()</code>
 *   <li><code>new String('hello world')</code>
 *   <li><code>new int[] {1,2,3,4}</code>
 *   <li><code>new String[] {'abc','xyz'}</code>
 *   <li><code>new int[5]</code>
 *   <li><code>new int[3][4]</code>
 * </ul>
 *
 * @author Andy Clement
...
public class ConstructorReference extends SpelNodeImpl {
  ...
  /**
   * Maximum number of elements permitted in an array declaration, applying to one-dimensional as
   * well as multi-dimensional arrays.
   *
   * @since 5.3.17
   */
   ...
   *
   * <p>The first argument is the type. The rest are the arguments to the constructor.
   */
   ...
   *
   * <p>The first argument is the array component type. The second argument is an {@link InlineList}
   * representing the array initializer, if an initializer was supplied in the expression.
   */
  ...
  private TypedValue createNewInstance(ExpressionState state) throws EvaluationException {
    ...
    if (executorToUse != null) {
      try {
        ...
        // 2. the constructor invoked was not passed the arguments it expected and has become
        // 'stale'
  ...
  private TypedValue createArray(ExpressionState state) throws EvaluationException {
    ...
    if (state.getEvaluationContext().getConstructorResolvers().isEmpty()) {
      ...
      throw new SpelEvaluationException(
          getStartPosition(), SpelMessage.CONSTRUCTOR_NOT_FOUND, type + "[]", "[]");
    }
    ...
    if (!hasInitializer()) {
      ...
      newArray =
          switch (arrayTypeCode) {
            case OBJECT ->
                createReferenceTypeArray(state, typeConverter, initializer.children, componentType);
  ...
  private void checkNumElements(long numElements) {
    if (numElements >= MAX_ARRAY_ELEMENTS) {
      throw new SpelEvaluationException(
          getStartPosition(),
          SpelMessage.MAX_ARRAY_ELEMENTS_THRESHOLD_EXCEEDED,
          MAX_ARRAY_ELEMENTS);
    }
  }

  private Object createReferenceTypeArray(
  ...
  private boolean[] createBooleanArray(
      ExpressionState state, TypeConverter typeConverter, SpelNodeImpl[] children) {
  ...
  private byte[] createByteArray(
      ExpressionState state, TypeConverter converter, SpelNodeImpl[] children) {
