 ...
 *
 * <p>Each pattern may be a simple path which has a one-to-one mapping to a resource on the
 * classpath, or alternatively may contain the special {@code *} character to indicate a wildcard
 * match. For example:
 *
 * <ul>
 *   <li>{@code file.properties}: matches just the {@code file.properties} file at the root of the
 *       classpath.
 *   <li>{@code com/example/file.properties}: matches just the {@code file.properties} file in
 *       {@code com/example/}.
 *   <li>{@code *.properties}: matches all the files with a {@code .properties} extension anywhere
 *       in the classpath.
 ...
 *   <li>{@code com/example/*}: matches all the files in {@code com/example/} and its child
 *       directories at any depth.
 * </ul>
 *
 * <p>A resource pattern must not start with a slash ({@code /}) unless it is the root directory.
 *
...
public final class ResourcePatternHint implements ConditionalHint {
  ...
  ResourcePatternHint(String pattern, @Nullable TypeReference reachableType) {
    Assert.isTrue(
        ("/".equals(pattern) || !pattern.startsWith("/")),
        () ->
