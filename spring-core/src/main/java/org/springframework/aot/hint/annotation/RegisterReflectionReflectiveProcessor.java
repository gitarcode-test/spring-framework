...
/**
 * A {@link ReflectiveProcessor} implementation that pairs with {@link
 * RegisterReflection @RegisterReflection}. Can be used as a base implementation for composed
 * annotations that are meta-annotated with {@link RegisterReflection}.
 *
...
public class RegisterReflectionReflectiveProcessor implements ReflectiveProcessor {
  ...
  public final void registerReflectionHints(ReflectionHints hints, AnnotatedElement element) {
    RegisterReflection annotation =
        AnnotatedElementUtils.getMergedAnnotation(element, RegisterReflection.class);
    Assert.notNull(
        annotation,
        "Element must be annotated with @"
            + RegisterReflection.class.getSimpleName()
            + ": "
            + element);
      ...
      if (element instanceof Class<?> clazz) {
        ...
        throw new IllegalStateException(
            "At least one class must be specified, "
                + "could not detect target from '"
                + element
                + "'");
      }
    }
    return new ReflectionRegistration(
        allClassNames.toArray(new Class<?>[0]), annotation.memberCategories());
  }

  protected void registerReflectionHints(
      ReflectionHints hints, ReflectionRegistration registration) {
  ...
  protected void registerReflectionHints(
      ReflectionHints hints, Class<?> target, MemberCategory[] memberCategories) {
