 ...
 * JPA {@link jakarta.persistence.EntityManagerFactory}. Binds a JPA EntityManager from the
 * specified factory to the thread, potentially allowing for one thread-bound EntityManager per
 * factory. {@link SharedEntityManagerCreator} and {@code @PersistenceContext} are aware of
 * thread-bound entity managers and participate in such transactions automatically. Using either is
 * required for JPA access code supporting this transaction management mechanism.
 *
 ...
 *
 * <p>This transaction manager supports nested transactions via JDBC Savepoints. The {@link
...
public class JpaTransactionManager extends AbstractPlatformTransactionManager
  ...
  @Nullable private Consumer<EntityManager> entityManagerInitializer;
  ...
  /**
   * Allow {@code Map} access to the JPA properties to be passed to the persistence provider, with
  ...
  /**
   * Specify a callback for customizing every {@code EntityManager} resource created for a new
   * transaction managed by this {@code JpaTransactionManager}.
   *
   * <p>This is an alternative to a factory-level {@code EntityManager} customizer and to a {@code
   * JpaVendorAdapter}-level {@code postProcessEntityManager} callback, enabling specific
   * customizations of transactional resources.
   *
   * @since 5.3
   ...
   * delegates to {@link EntityManagerFactoryInfo#createNativeEntityManager} if possible which in
   * turns applies {@link JpaVendorAdapter#postProcessEntityManager(EntityManager)}.
   *
