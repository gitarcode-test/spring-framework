...
public final class PersistenceManagedTypesScanner {
  ...
  @Nullable private final CandidateComponentsIndex componentsIndex;
   ...
   * @since 6.1.4
   */
  public PersistenceManagedTypesScanner(
      ResourceLoader resourceLoader, @Nullable ManagedClassNameFilter managedClassNameFilter) {
    ...
    this.managedClassNameFilter =
        (managedClassNameFilter != null ? managedClassNameFilter : className -> true);
  }

  /**
   * Scan the specified packages and return a {@link PersistenceManagedTypes} that represents the
  ...
  private void scanPackage(String pkg, ScanResult scanResult) {
    if (this.componentsIndex != null) {
      ...
      scanResult.managedPackages.addAll(
      ...
      for (Resource resource : resources) {
        try {
          ...
          if (matchesEntityTypeFilter(reader, factory)
            ...
            scanResult.managedPackages.add(
   ...
   */
  private boolean matchesEntityTypeFilter(MetadataReader reader, MetadataReaderFactory factory)
      throws IOException {
  ...
  private static class ScanResult {
    ...
    PersistenceManagedTypes toJpaManagedTypes() {
      return new SimplePersistenceManagedTypes(
          this.managedClassNames, this.managedPackages, this.persistenceUnitRootUrl);
    }
  }
}
