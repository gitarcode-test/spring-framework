 ...
 *
 * <p>Can also be used for manual data binding. Simply instantiate a ServletRequestDataBinder for
 * each binding process, and invoke {@code bind} with the current ServletRequest as argument:
 *
 * <pre class="code">
...
public class ServletRequestDataBinder extends WebDataBinder {
  ...
  /**
   * Use a default or single data constructor to create the target by binding request parameters,
   * multipart files, or parts to constructor args.
   *
   * <p>After the call, use {@link #getBindingResult()} to check for bind errors. If there are none,
   * the target is set, and {@link #bind(ServletRequest)} can be called for further initialization
   * via setters.
   *
   ...
   * String. Servlet Part binding is also supported when the request has not been parsed to
   * MultipartRequest via MultipartResolver.
   *
  ...
  private static boolean isFormDataPost(ServletRequest request) {
    return StringUtils.startsWithIgnoreCase(
        request.getContentType(), MediaType.MULTIPART_FORM_DATA_VALUE);
  }
  ...
  /**
   * Return a {@code ServletRequest} {@link ValueResolver}. Mainly for use from {@link
  ...
  protected static class ServletRequestValueResolver implements ValueResolver {
    ...
    @Nullable private Set<String> parameterNames;
    ...
    protected Object getRequestParameter(String name, Class<?> type) {
      ...
      return (ObjectUtils.isArray(value) && Array.getLength(value) == 1
          ? Array.get(value, 0)
          : value);
    }
    ...
    private Object getMultipartValue(String name) {
      MultipartRequest multipartRequest =
          WebUtils.getNativeRequest(this.request, MultipartRequest.class);
      if (multipartRequest != null) {
        ...
        HttpServletRequest httpRequest =
            WebUtils.getNativeRequest(this.request, HttpServletRequest.class);
