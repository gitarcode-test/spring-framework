...
/**
 * Implements the invocation of an {@link HttpExchange @HttpExchange}-annotated, {@link
 * HttpServiceProxyFactory#createClient(Class) HTTP service proxy} method by delegating to an {@link
 * HttpExchangeAdapter} to perform actual requests.
 *
...
final class HttpServiceMethod {
  ...
  public Object invoke(Object[] arguments) {
    HttpRequestValues.Builder requestValues =
        this.requestValuesInitializer.initializeRequestValuesBuilder();
      ...
      Assert.state(
          ...
          () ->
              "Could not resolve parameter ["
                  + this.parameters[index].getParameterIndex()
                  + "] in "
  ...
  /**
   * Factory for {@link HttpRequestValues} with values extracted from the type and method-level
   * {@link HttpExchange @HttpExchange} annotations.
   */
  private record HttpRequestValuesInitializer(
    ...
    public static HttpRequestValuesInitializer create(
      ...
      HttpExchange typeAnnotation =
          (!typeHttpExchanges.isEmpty() ? typeHttpExchanges.get(0).httpExchange : null);
    ...
    @Nullable
    private static HttpMethod initHttpMethod(
    ...
    private static String initUrl(
        @Nullable HttpExchange typeAnnotation,
        HttpExchange methodAnnotation,
        @Nullable StringValueResolver embeddedValueResolver) {
      ...
      if (embeddedValueResolver != null) {
        typeLevelUrl =
            (typeLevelUrl != null ? embeddedValueResolver.resolveStringValue(typeLevelUrl) : null);
      ...
      if (hasTypeLevelUrl && hasMethodLevelUrl) {
        return (typeLevelUrl
            + (!typeLevelUrl.endsWith("/") && !methodLevelUrl.startsWith("/") ? "/" : "")
            + methodLevelUrl);
      }
    ...
    @Nullable
    private static MediaType initContentType(
        @Nullable HttpExchange typeAnnotation, HttpExchange methodAnnotation) {
    ...
    @Nullable
    private static List<MediaType> initAccept(
        @Nullable HttpExchange typeAnnotation, HttpExchange methodAnnotation) {
  ...
  private record ExchangeResponseFunction(Function<HttpRequestValues, Object> responseFunction)
    ...
    public static ResponseFunction create(HttpExchangeAdapter client, Method method) {
      ...
      if (ClassUtils.isVoidType(paramType)) {
        ...
        responseFunction =
            request -> asOptionalIfNecessary(client.exchangeForHeaders(request), param);
    ...
    private static @Nullable Object asOptionalIfNecessary(
        @Nullable Object response, MethodParameter param) {
      return param.getParameterType().equals(Optional.class)
  ...
  private record ReactorExchangeResponseFunction(
    ...
    public static ResponseFunction create(ReactorHttpExchangeAdapter client, Method method) {
      ...
      Class<?> actualType =
          isSuspending ? actualParam.getParameterType() : actualParam.getNestedParameterType();
      ...
      if (ClassUtils.isVoidType(actualType)) {
        ...
        if (bodyType.equals(Void.class)) {
          ...
          responseFunction =
              initResponseEntityFunction(client, bodyParam, bodyAdapter, isSuspending);
        }
      } else {
    ...
    private static Function<HttpRequestValues, Publisher<?>> initResponseEntityFunction(
      ...
      return request ->
          client
              .exchangeForEntityFlux(request, bodyType)
              .map(
                  entity -> {
