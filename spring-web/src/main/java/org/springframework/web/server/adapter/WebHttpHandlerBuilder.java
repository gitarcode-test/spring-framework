...
public final class WebHttpHandlerBuilder {
  ...
  @Nullable private Function<HttpHandler, HttpHandler> httpHandlerDecorator;
  ...
  @Nullable private ServerRequestObservationConvention observationConvention;
   ...
   *   <li>{@link HttpHandlerDecoratorFactory} [0..N] -- detected by type and ordered.
   *   <li>{@link ObservationRegistry} -- detected by type and configured if unique.
   *   <li>{@link ServerRequestObservationConvention} -- detected by type and configured if unique.
  ...
  public static WebHttpHandlerBuilder applicationContext(ApplicationContext context) {
    ...
    List<WebExceptionHandler> exceptionHandlers =
        context.getBeanProvider(WebExceptionHandler.class).orderedStream().toList();
    ...
    context
        .getBeanProvider(ServerRequestObservationConvention.class)
        .ifAvailable(builder::observationConvention);
  ...
  /**
   * Configure an {@link ObservationRegistry} for recording server exchange observations. By
   * default, a {@link ObservationRegistry#NOOP no-op} registry will be configured.
   *
  ...
  /**
   * Configure a {@link ServerRequestObservationConvention} to use for server observations. By
   * default, a {@link DefaultServerRequestObservationConvention} will be used.
   *
   * @param observationConvention the convention to use for all recorded observations
   * @since 6.1
   */
  public WebHttpHandlerBuilder observationConvention(
  ...
  /**
   * Configure a {@link Function} to decorate the {@link HttpHandler} returned by this builder which
   * effectively wraps the entire {@link WebExceptionHandler} - {@link WebFilter} - {@link
   * WebHandler} processing chain. This provides access to the request and response before the
   * entire chain and likewise the ability to observe the result of the entire chain.
   *
   * @param handlerDecorator the decorator to apply
   * @since 5.3
   */
  public WebHttpHandlerBuilder httpHandlerDecorator(
      Function<HttpHandler, HttpHandler> handlerDecorator) {
    this.httpHandlerDecorator =
        (this.httpHandlerDecorator != null
            ? handlerDecorator.andThen(this.httpHandlerDecorator)
            : handlerDecorator);
    return this;
  }

  /**
   * Whether a decorator for {@link HttpHandler} is configured or not via {@link
   * #httpHandlerDecorator(Function)}.
   *
   * @since 5.3
   */
  ...
  public static class SpringWebBlockHoundIntegration implements BlockHoundIntegration {
    ...
    public void applyTo(BlockHound.Builder builder) {
      ...
      // Avoid hard references potentially anywhere in spring-web (no need for structural
      // dependency)
