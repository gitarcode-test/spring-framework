...
public class SpringCacheAnnotationParser implements CacheAnnotationParser, Serializable {
  ...
  private Collection<CacheOperation> parseCacheAnnotations(
    ...
    annotations.stream()
        ...
        .forEach(cacheable -> ops.add(parseCacheableAnnotation(ae, cachingConfig, cacheable)));
    annotations.stream()
        .filter(CachePut.class::isInstance)
        .map(CachePut.class::cast)
        .forEach(cachePut -> ops.add(parsePutAnnotation(ae, cachingConfig, cachePut)));
    annotations.stream()
        .filter(Caching.class::isInstance)
        .map(Caching.class::cast)
        .forEach(caching -> parseCachingAnnotation(ae, cachingConfig, caching, ops));
    return ops;
  }
   ...
   * might be multiple sources for default values, this ensures that the operation is in a proper
  ...
  private void validateCacheOperation(AnnotatedElement ae, CacheOperation operation) {
    ...
    if (StringUtils.hasText(operation.getCacheManager())
      ...
      throw new IllegalStateException(
          "Invalid cache annotation configuration on '"
              + ae.toString()
              + "'. Both 'cacheManager' and 'cacheResolver' attributes have been set. These"
              + " attributes are mutually exclusive: the cache manager is used to configure"
              + " adefault cache resolver if none is set. If a cache resolver is set, the cache"
              + " managerwon't be used.");
    }
  }
