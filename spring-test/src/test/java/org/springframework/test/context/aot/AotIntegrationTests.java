...
class AotIntegrationTests extends AbstractAotTests {
  ...
  void endToEndTests() {
    ...
    Stream<Class<?>> testClasses =
        createTestClassScanner().scan("org.springframework.test.context.aot.samples.basic");

    // AOT BUILD-TIME: PROCESSING
    InMemoryGeneratedFiles generatedFiles = new InMemoryGeneratedFiles();
    TestContextAotGenerator generator =
        new TestContextAotGenerator(generatedFiles, new RuntimeHints());
    generator.processAheadOfTime(testClasses);

    List<String> sourceFiles =
        generatedFiles.getGeneratedFiles(Kind.SOURCE).keySet().stream().toList();
            ...
            compiled ->
                ...
                runTestsInAotMode(
                    ...
                    List.of(
                        ...
                        /* 2 */ BasicSpringJupiterTests
                            .class, // NestedTests get executed automatically
    ...
    List<Class<?>> testClasses =
        List.of(
            org.springframework.test.context.bean.override.convention
                .TestBeanForByNameLookupIntegrationTests.class,
            org.springframework.test.context.bean.override.mockito
                .MockitoBeanForByNameLookupIntegrationTests.class,
  ...
  private void runEndToEndTests(List<Class<?>> testClasses, boolean failOnError) {
    ...
    InMemoryGeneratedFiles generatedFiles = new InMemoryGeneratedFiles();
    TestContextAotGenerator generator =
        new TestContextAotGenerator(generatedFiles, new RuntimeHints(), failOnError);
    ...
    try {
      ...
      if (summary.getTotalFailureCount() > 0) {
        ...
        List<Throwable> exceptions =
            summary.getFailures().stream().map(Failure::getException).toList();
