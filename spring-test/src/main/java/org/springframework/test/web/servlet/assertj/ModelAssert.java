...
/**
 * AssertJ {@linkplain org.assertj.core.api.Assert assertions} that can be applied to a {@linkplain
 * ModelAndView#getModel() model}.
 *
...
public class ModelAssert
  ...
  /**
   * Return a new {@linkplain AbstractBindingResultAssert assertion} object that uses the {@link
   * BindingResult} with the given {@code name} as the object to test.
   *
   * <p>Example:
   *
   * <pre><code class="java">
  ...
  /**
   * Verify that the actual model contains the attributes with the given {@code names}, and that
   * each of these attributes has each at least one error.
   *
  ...
  public ModelAssert hasAttributeErrors(String... names) {
    return assertAttributes(
        ...
        "to have attribute errors for",
        "these attributes do not have any errors");
  }

  /**
   * Verify that the actual model contains the attributes with the given {@code names}, and that
   * none of these attributes has an error.
   *
  ...
  public ModelAssert doesNotHaveAttributeErrors(String... names) {
    return assertAttributes(
        names,
        Predicate.not(BindingResult::hasErrors),
        "to have attribute without errors for",
        "these attributes have at least one error");
  }

  private ModelAssert assertAttributes(
      String[] names,
      Predicate<BindingResult> condition,
      String assertionMessage,
      String failAssertionMessage) {
    ...
    if (!missing.isEmpty() || !failCondition.isEmpty()) {
      ...
      if (!missing.isEmpty()) {
        sb.append(
            "%nbut could not find these attributes:%n  %s%n".formatted(String.join(", ", missing)));
      }
      if (!failCondition.isEmpty()) {
        String prefix = missing.isEmpty() ? "but" : "and";
        sb.append(
            "%n%s %s:%n  %s%n"
                .formatted(prefix, failAssertionMessage, String.join(", ", failCondition)));
      }
