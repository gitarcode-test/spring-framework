...
abstract class AbstractExpressionEvaluatingCondition implements ExecutionCondition {
   ...
   *     {@link ConditionEvaluationResult#enabled(String) enabled} if the expression evaluates to
   *     {@code true}
   * @param context the {@code ExtensionContext}
   * @return {@link ConditionEvaluationResult#enabled(String) enabled} if the container or test
   *     should be enabled; otherwise {@link ConditionEvaluationResult#disabled(String) disabled}
   */
  protected <A extends Annotation> ConditionEvaluationResult evaluateAnnotation(
    ...
    AnnotatedElement element =
        context.getElement().orElseThrow(() -> new IllegalStateException("No AnnotatedElement"));
    ...
    if (loadContext && result.isDisabled() && element instanceof Class<?> testClass) {
      DirtiesContext dirtiesContext =
          TestContextAnnotationUtils.findMergedAnnotation(testClass, DirtiesContext.class);
