...
/**
 * The default implementation of {@link DatabaseClient}, as created by the static factory method.
 *
 * @author Mark Paluch
 * @author Mingyuan Wu
...
final class DefaultDatabaseClient implements DatabaseClient {
  ...
  @Nullable private final NamedParameterExpander namedParameterExpander;
  ...
  public <T> Mono<T> inConnection(Function<Connection, Mono<T>> action) {
    ...
    Mono<ConnectionCloseHolder> connectionMono =
        getConnection()
    ...
    return Mono.usingWhen(
            ...
            connectionCloseHolder -> {
              ...
              try {
                ...
                return Mono.error(
                    ConnectionFactoryUtils.convertR2dbcException("doInConnection", sql, ex));
              }
            },
    ...
    return Flux.usingWhen(
            ...
            connectionCloseHolder -> {
              ...
              try {
                ...
                String sql = getSql(action);
                return Flux.error(
                    ConnectionFactoryUtils.convertR2dbcException("doInConnectionMany", sql, ex));
              }
            },
            ConnectionCloseHolder::close,
            (it, err) -> it.close(),
  ...
  /**
   * Create a close-suppressing proxy for the given R2DBC Connection. Called by the {@code execute}
   * method.
   *
  ...
  class DefaultGenericExecuteSpec implements GenericExecuteSpec {
    ...
    public DefaultGenericExecuteSpec bind(int index, Object value) {
      ...
      Assert.notNull(
          value,
          () ->
              String.format("Value at index %d must not be null. Use bindNull(…) instead.", index));
      ...
      return new DefaultGenericExecuteSpec(
          byIndex, this.byName, this.sqlSupplier, this.filterFunction);
    }
    ...
    public DefaultGenericExecuteSpec bind(String name, Object value) {
      ...
      return new DefaultGenericExecuteSpec(
          this.byIndex, byName, this.sqlSupplier, this.filterFunction);
    }
    ...
    public GenericExecuteSpec bindValues(Map<String, ?> source) {
      ...
      return new DefaultGenericExecuteSpec(
          this.byIndex, target, this.sqlSupplier, this.filterFunction);
    }
    ...
    public DefaultGenericExecuteSpec bindProperties(Object source) {
      ...
      for (PropertyDescriptor pd : BeanUtils.getPropertyDescriptors(source.getClass())) {
        if (pd.getReadMethod() != null && pd.getReadMethod().getDeclaringClass() != Object.class) {
          ...
          byName.put(
              pd.getName(),
              (value != null ? Parameters.in(value) : Parameters.in(pd.getPropertyType())));
        }
      }
    ...
    private ResultFunction getResultFunction(Supplier<String> sqlSupplier) {
      BiFunction<Connection, String, Statement> statementFunction =
          (connection, sql) -> {
            ...
            if (DefaultDatabaseClient.this.namedParameterExpander != null) {
              ...
              List<String> parameterNames =
                  DefaultDatabaseClient.this.namedParameterExpander.getParameterNames(sql);
              MapBindParameterSource namedBindings =
                  retrieveParameters(sql, parameterNames, remainderByName, remainderByIndex);

              PreparedOperation<String> operation =
                  DefaultDatabaseClient.this.namedParameterExpander.expand(
    ...
    private <T> FetchSpec<T> execute(
        Supplier<String> sqlSupplier, Function<Result, Publisher<T>> resultAdapter) {
    ...
    private <T> Flux<T> flatMap(
        Supplier<String> sqlSupplier, Function<Result, Publisher<T>> mappingFunction) {
      ResultFunction resultHandler = getResultFunction(sqlSupplier);
      ConnectionFunction<Flux<T>> connectionFunction =
          new DelegateConnectionFunction<>(
              resultHandler, cx -> resultHandler.apply(cx).flatMap(mappingFunction));
      return inConnectionMany(connectionFunction);
    }

    private MapBindParameterSource retrieveParameters(
        String sql,
        List<String> parameterNames,
        Map<String, Parameter> remainderByName,
        Map<Integer, Parameter> remainderByIndex) {

      Map<String, Parameter> namedBindings =
          CollectionUtils.newLinkedHashMap(parameterNames.size());
      for (String parameterName : parameterNames) {
        Parameter parameter =
            getParameter(remainderByName, remainderByIndex, parameterNames, parameterName);
        if (parameter == null) {
  ...
  /**
   * Invocation handler that suppresses close calls on R2DBC Connections. Also prepares returned
  ...
  static class ConnectionCloseHolder extends AtomicBoolean {
    ...
    ConnectionCloseHolder(
        Connection connection, Function<Connection, Publisher<Void>> closeFunction) {
